...
```{r} archiveFile <- "repdata-data-activity.zip"
...
```{r} unzip("repdata-data-activity.zip") ```
```{r} activity <- read.csv(file = "activity.csv")```
...
```{r} activity$day <- factor(paste(substring(months(as.Date(activity$date)),0,3),
substring(as.Date  (activity$date), 9), sep=" ")) ```

```{r} tidyData <- na.omit(activity) ```

```{r} head(tidyData)```

```{r}activity$date <- as.Date(activity$date, format="%Y-%m-%d") ```

### What is mean total number of steps taken per day?  

The next step is to summarize and plot the data to get steps per day.

```{r} tot_steps <- aggregate(tidyData$steps, list(tidyData$date), FUN="sum")```
```{r} names(tot_steps) <- c("date","total")```
```{r} tot_steps$date <- factor(paste(substring(months(as.Date(tot_steps$date)),0,3), 
substring(as.Date  (tot_steps$date), 9), sep=" ")) ```
```{r} barplot(tot_steps$total, names.arg=tot_steps$date, xlab="Days", ylab="Total Steps", main="Total Steps Count Per Day", col="red")```

The plot shows a significant difference in the number of steps per day.  In addition to a number of days where no steps are recorded, the peak number of steps are shown at the end of November. 

Looking at the **mean** and **median** number of steps taken per day: 
```{r} print(mean(tot_steps$total)) ```
```{r} print(median(tot_steps$total)) ```

### What is the average daily activity pattern?  
First step to answer this question is to get the mean of steps.  

```{r} activity_steps <- aggregate(tidyData$steps, list(as.numeric(tidyData$interval)), FUN="mean")```
```{r} names(activity_steps) <- c("interval","mean") ```

This is then plotted at 5-minute intervals (x-axis) and the average number of steps taken, averaged across all days (y-axis):

```{r} plot(activity_steps, type="l", xlab="Interval", ylab="Number of Steps", main="Daily Activity Pattern", col= "green") ```

### Which 5-minute interval, on average across all the days in the dataset, contains the maximum number of steps?

Find the row id, interval and mean of maximum average number of steps in a 5-minute interval. 

```{r} maxStepInt <- activity_steps[which.max(activity_steps$mean),]
print(maxStepInt) ```
...
```{r} tot_steps2 <- aggregate(steps ~ interval, data = activity, FUN = mean)
fillNA <- numeric()
...
```{r} tot_steps2 <- aggregate(new_activity$steps, list(new_activity$date), FUN="sum")
names(tot_steps2) <- c("date","total") ```
```{r} tot_steps2$date <- factor(paste(substring(months(as.Date(tot_steps2$date)),0,3), substring(as.Date  (tot_steps2$date), 9), sep=" ")) ```
```{r} barplot(tot_steps2$total, names.arg=tot_steps2$date, xlab="Days", ylab="Total Steps",main="Total Steps Count Per Day (W/o Missing Data)", col="steelblue") ```

Next step is to print the mean and median total steps without the missing data.  

```{r} print(mean(tot_steps2$total)) ```
```{r} print(median(tot_steps2$total)) ```
...
```{r} activity$date <- as.Date(activity$date, "%Y-%m-%d")  ```
```{r} activity$day <- weekdays(activity$date)  ```
```{r} activity$day_type <- c("weekday")  ```
```{r} for (i in 1:nrow(activity)){
if (activity$day[i] == "Saturday" || activity$day[i] == "Sunday"){
activity$day_type[i] <- "weekend"
}}  ```
```{r} activity$day_type <- as.factor(activity$day_type)  ```
```{r} table_interval_steps_imputed <- aggregate(steps ~ interval+day_type, activity, mean)  ``` 
```{r} qplot(interval, steps, data=table_interval_steps_imputed, geom=c("line"), xlab="Interval", 
